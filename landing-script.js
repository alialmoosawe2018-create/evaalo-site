...
// ====================================
// Mobile Card Animation - Draw Border on Click
// ====================================

function initMobileCardAnimation() {
    // Only apply on mobile devices
    if (window.innerWidth > 768) return;
    
    const cards = document.querySelectorAll('#features .simple-icon-wrapper');
    
    cards.forEach((card, index) => {
        // Set initial collapsed state
        card.classList.add('collapsed');
        
        // Create border elements for horizontal lines
        ...
        card.addEventListener('click', function(e) {
            ...
            // If another card is expanding, wait
            if (this.classList.contains('expanding')) return;
            
            // Create ripple effect
            const ripple = document.createElement('div');
            ripple.classList.add('ripple');
            const iconWrapper = this.querySelector('.icon-wrapper-inner');
            if (iconWrapper) {
                const rect = iconWrapper.getBoundingClientRect();
                const cardRect = this.getBoundingClientRect();
                const size = Math.max(rect.width, rect.height);
                ripple.style.width = ripple.style.height = size + 'px';
                ripple.style.left = (rect.left - cardRect.left + rect.width / 2 - size / 2) + 'px';
                ripple.style.top = (rect.top - cardRect.top + rect.height / 2 - size / 2) + 'px';
            }
            ...
            for (let i = 0; i < 8; i++) {
                ...
                if (iconWrapper) {
                    const rect = iconWrapper.getBoundingClientRect();
                    const cardRect = this.getBoundingClientRect();
                    sparkle.style.left = (rect.left - cardRect.left + rect.width / 2) + 'px';
                    sparkle.style.top = (rect.top - cardRect.top + rect.height / 2) + 'px';
                }
            ...
            // After all animations complete, set to expanded state
            setTimeout(() => {
                ...
                // Hide animated borders and keep original border
                const borderTop = this.querySelector('.border-top');
                const borderBottom = this.querySelector('.border-bottom');
                if (borderTop) borderTop.style.display = 'none';
                if (borderBottom) borderBottom.style.display = 'none';
                
                // Remove pseudo-element borders by setting them to transparent
                this.style.setProperty('--before-opacity', '0');
                this.style.setProperty('--after-opacity', '0');
                
                // Add completion pulse effect
                this.style.animation = 'completePulse 0.4s ease-out';
                setTimeout(() => {
                    this.style.animation = '';
                }, 400);
            }, 2000); // Total animation time: 0.7s (vertical) + 0.6s (horizontal) + 0.7s (content)
        });
        
        // Add staggered appearance for icons
        setTimeout(() => {
            const iconWrapper = card.querySelector('.icon-wrapper-inner');
            if (iconWrapper) {
                iconWrapper.style.opacity = '0';
                iconWrapper.style.transform = 'scale(0.5)';
                iconWrapper.style.transition = 'all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)';
...
window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(() => {
        // Reset if switching between mobile and desktop
        const cards = document.querySelectorAll('#features .simple-icon-wrapper');
        if (window.innerWidth > 768) {
            cards.forEach(card => {
                card.classList.remove('collapsed', 'expanding', 'expanded');
                const borders = card.querySelectorAll('.border-top, .border-bottom');
                borders.forEach(b => b.remove());
            });
        } else {
            // Re-initialize for mobile
            const borders = document.querySelectorAll('.border-top, .border-bottom');
            if (borders.length === 0) {
                initMobileCardAnimation();
            }
        }
    }, 250);
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize mobile card animation
    setTimeout(() => {
        initMobileCardAnimation();
    }, 100);
});
