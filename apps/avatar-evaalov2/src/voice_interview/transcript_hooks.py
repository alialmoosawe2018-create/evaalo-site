...
from voice_interview.assistant import TtsRouteContext
...
def attach_user_transcript_routing(
    ...
    *,
    ...
    min_user_chars = int(os.getenv("MIN_USER_TRANSCRIPT_CHARS", "2" if interview_defaults else "3"))
    # Short replies ("لا", "نعم", "ok") must pass through; floor keeps env from going too low by mistake.
    try:
        iv_floor = int(os.getenv("MIN_USER_TRANSCRIPT_CHARS_INTERVIEW_FLOOR", "2"))
    except ValueError:
        iv_floor = 4
    iv_floor = max(2, min(12, iv_floor))
    if interview_defaults and min_user_chars < iv_floor:
        logger.debug("MIN_USER_TRANSCRIPT_CHARS %d -> %d (interview floor)", min_user_chars, iv_floor)
    ...
    lang_switch_min_chars = int(os.getenv("TTS_LANG_SWITCH_MIN_CHARS", "4" if interview_defaults else "6"))
    iv_lang_floor = 4
    if interview_defaults and lang_switch_min_chars < iv_lang_floor:
    ...
    # Code-switching guard: a candidate speaking Arabic with one English term must NOT
    ...
    def on_user_input_transcribed(ev: UserInputTranscribedEvent) -> None:
        ...
        # the hint that ``InterviewAssistant.tts_node`` uses to force the voice on
