...
When reception voice STT defaults are on, ``worker.run()`` sets
...
def configure_and_apply_playback_patches() -> None:
    """Reception voice defaults: opt in to suppress unless user set PLAYBACK_PATCH_* explicitly (empty = use default)."""
    try:
        from reception.config import reception_voice_defaults_enabled
...
def apply_playback_patches() -> None:
    ...
    if not suppress:
        logger.debug(
            ...
            "(worker sets default on when SPEECHMATICS_RECEPTION_DEFAULTS or legacy SPEECHMATICS_INTERVIEW_DEFAULTS) "
            "or raise AVATAR_CLEAR_BUFFER_TIMEOUT"
        )
        return
    if _PATCH_APPLIED:
        return
