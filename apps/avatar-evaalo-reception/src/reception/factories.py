...
try:
    ...
    # Evaalo product / demo vocabulary (Arabic + English loanwords). No HR interview bank.
    EVAALO_AR_VOCAB = [
        AdditionalVocabEntry(content="ایڤالو", sounds_like=["ايفالو", "Evaalo", "Evalo", "إيفالو", "ایvالo"]),
        AdditionalVocabEntry(content="ذكاء اصطناعي", sounds_like=["ذكاءاصطناعي", "اي اي"]),
        AdditionalVocabEntry(content="مقابلة", sounds_like=["مقابله", "انترفيو"]),
        AdditionalVocabEntry(content="أفاتار", sounds_like=["افاتار", "avatar", "أفاتار"]),
        AdditionalVocabEntry(content="توظيف", sounds_like=["التوظيف", "توظيف"]),
        AdditionalVocabEntry(content="أتمتة", sounds_like=["اتمتة", "automation"]),
        AdditionalVocabEntry(content="خدمة", sounds_like=["خدمه"]),
        AdditionalVocabEntry(content="تجربة", sounds_like=["تجربه", "ديمو"]),
    ]
    EVAALO_EN_VOCAB = [
        AdditionalVocabEntry(content="Evaalo", sounds_like=["ایڤالو", "إيفالو", "ايفالو", "ایvالو"]),
        AdditionalVocabEntry(content="automation", sounds_like=["اوتوميشن", "اتوميشن"]),
        AdditionalVocabEntry(content="recruitment", sounds_like=["ريكروتمنت", "توظيف"]),
        AdditionalVocabEntry(content="interview", sounds_like=["انترفيو", "مقابلة"]),
        AdditionalVocabEntry(content="voice agent", sounds_like=["فويس ايجنت", "وكيل صوتي"]),
        AdditionalVocabEntry(content="video agent", sounds_like=["فيديو ايجنت", "وكيل فيديو"]),
        AdditionalVocabEntry(content="LiveKit", sounds_like=["لايف كت", "ليف كت"]),
        AdditionalVocabEntry(content="demo", sounds_like=["ديمو", "تجربة"]),
        AdditionalVocabEntry(content="HR", sounds_like=["اتش ار", "موارد بشرية", "human resources"]),
    ]
except ImportError:
...
def load_vad():
    # Reception parity with the production interview path: default to STT-only EOU (Speechmatics)
    # and skip Silero VAD + multilingual turn detector to cut endpointing latency. LiveKit still
    # supports Silero VAD + turn detector for natural turn-taking when explicitly enabled via
    # ``DISABLE_VAD=false`` (see https://docs.livekit.io/agents/logic/turns/turn-detector/).
...
def maybe_warmup_elevenlabs() -> None:
    ...
    if os.getenv("ELEVENLABS_WARMUP_ON_WORKER_START", "false").strip().lower() not in (
        ...
        return
    api_key = (os.getenv("ELEVENLABS_RECEPTION_API_KEY") or os.getenv("ELEVENLABS_API_KEY") or "").strip()
    if not api_key:
...
def create_openai_llm():
    api_key = (os.getenv("OPENAI_RECEPTION_API_KEY") or os.getenv("OPENAI_API_KEY") or "").strip()
    ...
    # Reception default: generous cap so the agent can finish thoughts (a typical Q&A about Evaalo
    # services lands around 90–160 tokens). Many deploys still pin OPENAI_MAX_COMPLETION_TOKENS to
    # 40/48 from older latency tuning — OpenAI then returns ``finish_reason=length`` and the reply
    # truncates mid-sentence (the log shows ``completion=40`` on every turn and a stray Arabic
    # letter at the end). To avoid silently shipping cut-off answers, clamp any cap below the
    # reception soft floor up to a safe minimum. Set RECEPTION_ALLOW_LOW_OPENAI_COMPLETION=1 to
    # genuinely keep a low cap (e.g. for cost experiments).
    raw_max = (os.getenv("OPENAI_MAX_COMPLETION_TOKENS", "256") or "").strip()
    cap: int = 0
    if raw_max and raw_max != "0":
        try:
            ...
            cap = 0
    if cap > 0:
        allow_low_cap = (os.getenv("RECEPTION_ALLOW_LOW_OPENAI_COMPLETION") or "").strip().lower() in (
            "1",
            "true",
            "yes",
        )
        try:
            soft_min = int((os.getenv("RECEPTION_OPENAI_COMPLETION_SOFT_MIN") or "128").strip())
        except ValueError:
            soft_min = 128
        soft_min = max(32, min(2048, soft_min))
        try:
            clamp_target = int((os.getenv("OPENAI_MAX_COMPLETION_TOKENS_CLAMP") or "256").strip())
        except ValueError:
            clamp_target = 256
        clamp_target = max(soft_min, min(4096, clamp_target))
        if (not allow_low_cap) and cap < soft_min:
            logger.warning(
                "OPENAI_MAX_COMPLETION_TOKENS=%d is below reception soft_min=%d — answers truncate "
                "(LLM returns finish_reason=length). Raising to %d. Fix deploy env to "
                "OPENAI_MAX_COMPLETION_TOKENS>=%d, OR set RECEPTION_ALLOW_LOW_OPENAI_COMPLETION=1 "
                "to keep the original low cap.",
                cap,
                soft_min,
                clamp_target,
                soft_min,
...
def _optional_elevenlabs_voice_settings() -> VoiceSettings | None:
    """Build VoiceSettings tuned for Arabic-first voice clarity.
...
def create_elevenlabs_tts():
    voice_id = (
        os.getenv("ELEVENLABS_RECEPTION_VOICE_ID") or os.getenv("ELEVENLABS_VOICE_ID") or ""
    ).strip() or ARABIC_VOICE_ID
    ...
    eleven_api_key = (
        (os.getenv("ELEVENLABS_RECEPTION_API_KEY") or "").strip()
...
def create_speechmatics_stt():
    ...
    ``ar_en`` reception sessions we default FIXED unless you override.
    """
    api_key = (os.getenv("SPEECHMATICS_RECEPTION_API_KEY") or os.getenv("SPEECHMATICS_API_KEY") or "").strip()
    ...
    reception_voice_boost = reception_voice_defaults_enabled()
    # Fixed STT pack: honor SPEECHMATICS_EOU_SILENCE / SPEECHMATICS_MAX_DELAY without reception-boost floors (e.g. 0.6 / 0.9).
    stt_fixed_raw = env_or(
        "SPEECHMATICS_RECEPTION_FIXED_STT",
        "SPEECHMATICS_INTERVIEW_FIXED_STT",
        default="true",
    )
    stt_fixed_pack = stt_fixed_raw.lower() in ("1", "true", "yes")
    ...
    # Default FIXED for reception+ar_en unless SPEECHMATICS_EOU_MODE is set or SPEECHMATICS_AR_EN_DEFAULT_FIXED_EOU=false.
    ...
    elif reception_voice_boost and _ar_en_pack and _ar_en_fixed_default:
        eou_mode_raw = "fixed"
        logger.debug(
            "speechmatics reception: ar_en default EOU=fixed (override with SPEECHMATICS_EOU_MODE=adaptive)"
        )
    else:
        eou_mode_raw = "adaptive" if reception_voice_boost else "fixed"

    # ar_en + reception defaults: adaptive EOU is often the source of repeated fallback EOU and jittery handoffs.
    ...
    if reception_voice_boost and _ar_en_pack and eou_mode_raw == "adaptive" and not allow_adaptive_ar_en:
        logger.warning(
            "speechmatics reception: forcing ar_en EOU to fixed (set SPEECHMATICS_AR_EN_ALLOW_ADAPTIVE=true to keep adaptive)"
        )
        eou_mode_raw = "fixed"
    ...
    if stt_fixed_pack:
        ...
        logger.debug(
            "speechmatics SPEECHMATICS_RECEPTION_FIXED_STT / INTERVIEW_FIXED_STT: max_delay=%.2f eou_silence=%.2f (fixed pack)",
            max_delay,
            eou_silence,
        )
    else:
    ...
    if reception_voice_boost and not stt_fixed_pack:
        ...
        if op_raw == "standard":
            logger.debug("speechmatics reception boost: operating_point enhanced (was standard)")
            op_raw = "enhanced"
        # Keep reception voice defaults responsive by default; allow explicit env override for stricter stability.
        # Latency (AVATAR_FAST_RESPONSE / RECEPTION_PROFILE=latency): lower target → faster finals after EOU.
        ...
        _target_max = float(
            env_or(
                "SPEECHMATICS_RECEPTION_TARGET_MAX_DELAY",
                "SPEECHMATICS_INTERVIEW_TARGET_MAX_DELAY",
                default=_target_max_default,
            )
        )
        if max_delay < _target_max:
            logger.debug(
                "speechmatics reception boost: max_delay=%.2f (responsive default; was %.2f)",
                _target_max,
                max_delay,
            )
        ...
        _min_eou = float(
            env_or(
                "SPEECHMATICS_RECEPTION_MIN_EOU_SILENCE",
                "SPEECHMATICS_INTERVIEW_MIN_EOU_SILENCE",
                default=_min_eou_default,
            )
        )
        if eou_silence < _min_eou:
            logger.debug("speechmatics reception boost: eou_silence=%.2f (was %.2f)", _min_eou, eou_silence)
            eou_silence = _min_eou
        if env_or(
            "SPEECHMATICS_RECEPTION_FORCE_FIXED_EOU",
            "SPEECHMATICS_INTERVIEW_FORCE_FIXED_EOU",
            default="false",
        ).lower() in (
            "1",
            "true",
            ...
            logger.debug(
                "speechmatics reception: eou_mode=fixed (SPEECHMATICS_RECEPTION_FORCE_FIXED_EOU/INTERVIEW_FORCE_FIXED_EOU; was %s)",
                eou_mode_raw,
            )
            eou_mode_raw = "fixed"
        ...
        if _stt_stable and eou_silence < 0.76:
            ...
            logger.debug(
                "speechmatics reception (strict): eou_silence -> %.2f (fewer micro-fragments)",
                eou_silence,
            )
        ...
        if eou_mode_raw == "adaptive":
            ...
            soft_cap = float(
                env_or(
                    "SPEECHMATICS_RECEPTION_ADAPTIVE_SOFT_CAP",
                    "SPEECHMATICS_INTERVIEW_ADAPTIVE_SOFT_CAP",
                    default="0.76",
                )
            )
            ...
            if eou_silence < soft_floor:
                logger.debug(
                    "speechmatics reception (adaptive): eou_silence %.2f -> %.2f (EOU floor; avatar latency)",
                    eou_silence,
                    soft_floor,
                )
            ...
            if eou_silence < floor:
                logger.debug(
                    "speechmatics reception (fixed): eou_silence %.2f -> %.2f (%s)",
                    eou_silence,
                    floor,
    ...
    # Real-time partial transcripts (streaming): on for fixed pack + explicit env; reception boost alone defaulted off.
    _partials_default = (
        "true"
        if (stt_fixed_pack or not reception_voice_boost)
        else "false"
    )
    ...
    # English product terms merged for ar_en packs. Disable:
    # SPEECHMATICS_DISABLE_EN_EVAALO_VOCAB=true
    disable_en_vocab = os.getenv("SPEECHMATICS_DISABLE_EN_EVAALO_VOCAB", "false").lower() in (
        "1",
        "true",
        "yes",
    )
...
def create_avatar_session() -> AvatarSession | None:
    api_key = (
        os.getenv("BEYOND_PRESENCE_RECEPTION_API_KEY") or os.getenv("BEYOND_PRESENCE_API_KEY") or ""
    ).strip()
    avatar_id = (
        os.getenv("BEYOND_PRESENCE_RECEPTION_AVATAR_ID") or os.getenv("BEYOND_PRESENCE_AVATAR_ID") or ""
    ).strip()
    api_url = (
        os.getenv("BEYOND_PRESENCE_RECEPTION_API_URL") or os.getenv("BEYOND_PRESENCE_API_URL") or ""
    ).strip() or None
