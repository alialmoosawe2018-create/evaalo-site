...
/*
 * شكل الموجة: فصّان متناظران — قمّة، فحوض في المنتصف، فقمّة.
 *
 * 🔴 ولماذا تسعة لا خمسة عشر. عرض خمسة عشر عموداً = ٨٩ (أعمدة) + ٥٦ (فجوات)
 * + ١٢ (حشو) = **١٥٧px**، بينما `.voice-interview-stage__waveform` تُعلن
 * `flex: 0 0 120px`. والفارق لا يُمتصّ: `overflow` مرئيّ، فالمحتوى **يفيض
 * ١٨px على كلّ جانب** — مقيسٌ في المتصفّح — وفجوة الصفّ ١٢px، فالفائض يتجاوزها
 * ويقع **فوق الاسم**. تراكبٌ حقيقيّ في الوضع العريض، مقيس: `overlap: true`.
 *
 * وتحت ٧٦٨px تُلغي قاعدةُ الوسائط السقف (`flex: 0 0 auto; width: auto`)، فلا
 * يقع تراكب لكنّ الصندوق يتمدّد إلى ١٥٧px ويقتطعها من الاسم وحده: في بطاقة
 * ٣٠٠px لا يبقى للاسم إلّا ٥٣px، فـ«حيدر ناضم» ينكسر على سطرين والاسم الكامل
 * على خمسة.
 *
 * فالعلّة واحدة في الوضعين: **المحتوى أوسع من صندوقه**. وتسعة أعمدة = ٩٧px،
 * تسع داخل ١٢٠px بلا فيض، وتترك للاسم مساحته في الضيّق. والموجة تبقى موجةً —
 * البديل المرفوض كان تضييق الأعمدة إلى ٣px فتصير خطّاً.
 */
const WAVE_SHAPE = [0.3, 0.65, 0.95, 0.6, 0.28, 0.6, 0.95, 0.65, 0.3];
...
const VoiceInterviewStage = ({
  ...
  // بعد أن يُنهي الخادم المقابلة (إغلاق 1000) نستبدل واجهة المقابلة بالكامل بشاشة
