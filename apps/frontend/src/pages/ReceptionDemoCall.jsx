...
/**
 * Reception demo — غطاء التحميل حتى يُربط فيديو Beyond (`avatarVideoSurfaceReady`).
 *
 * لماذا الأفاتار قد «يتأخر» (سلسلة كاملة، ليست فقط الواجهة):
 * 1) المتصفح: طلب الميكروفون/الكاميرا (getUserMedia).
 * 2) الباكند: إنشاء الغرفة + dispatch لوكيل `evaalo-reception-agent`.
 * 3) الوركر: قبول المهمة (أو بردة باردة)، `job_ctx.connect`، اكتمال ICE/TURN مع أقرب منطقة LiveKit.
 * 4) الوكيل: تشغيل AgentSession ثم AvatarSession؛ مشارك `bey-avatar-agent` ينضم كمسار ثانٍ وينشر الفيديو.
 * 5) العميل: `prepareConnection` + `room.connect`، اشتراك simulcast، أول إطار فيديو ثم `track.attach`.
 *
 * متغيرات (هذه الصفحة فقط): `VITE_RECEPTION_PREPARATION_COUNTDOWN_SECONDS` ثم fallback إلى
 * `VITE_PREPARATION_COUNTDOWN_SECONDS`. الافتراضي هنا 0 = سبينر فقط (بدون عدّ 30 ثانية مربك)
 * لأن الإخفاء الفعلي يعتمد على وصول الفيديو وليس على العدّ.
 */
const PREPARATION_COUNTDOWN_SECONDS = (() => {
    const receptionRaw = import.meta.env?.VITE_RECEPTION_PREPARATION_COUNTDOWN_SECONDS;
    const globalRaw = import.meta.env?.VITE_PREPARATION_COUNTDOWN_SECONDS;
    const raw =
        receptionRaw !== undefined && String(receptionRaw).trim() !== ''
            ? receptionRaw
            : globalRaw;
    const n =
...
/** ألوان حقول مودال الطلب — البطاقة تستخدم `.reception-page__card` */
const REC_DEMO_SHELL = {
    label: '#f1f5f9',
    radius: '8px',
};

function splitFullNameForLead(fullName) {
...
const ReceptionDemoCall = () => {
    ...
    /** بيانات الزائر لـ LiveKit job metadata (وكيل الاستقبال فقط) */
    const [receptionLead, setReceptionLead] = useState(null);
    const receptionLeadRef = useRef(null);
    ...
    /**
     * Pre-warm: ينشئ غرفة LiveKit + dispatch وكيل الاستقبال + Beyond مبكراً فور فتح صفحة الديمو،
     * قبل أن يبدأ المستخدم بتعبئة النموذج أو الضغط على "Start".
     *
     * فصل البيانات عن الجلسة: لا نُرسل بيانات النموذج هنا — الباكند يبني جلسة LiveKit على
     * أساس visitorId فقط ولا يضع البيانات الشخصية في metadata. /start لاحقاً يعيد استخدام
     * نفس الجلسة (لا استبدال) ويحفظ الـ lead في MongoDB.
     *
     * عطّله بـ VITE_DISABLE_RECEPTION_PREPARE=1.
     */
    useEffect(() => {
        if (!visitorId) return;
        if (import.meta.env.VITE_DISABLE_RECEPTION_PREPARE === '1') return;
        const controller = new AbortController();
        const t = setTimeout(() => {
            fetch(`${API_BASE}/api/reception-demo/prepare`, {
                method: 'POST',
                ...
                .catch((err) => {
                    if (err?.name !== 'AbortError' && import.meta.env.DEV) {
                        console.log('Reception prewarm skipped:', err?.message || err);
                    }
                });
        }, 250);
        return () => {
     ...
     * لو بدأ المستخدم المقابلة؛ إن لم يبدأ بعد نحاول sessionId من آخر استعداد (prewarmSessionIdRef إن وُجد).
     */
    useEffect(() => {
        const handlePageHide = () => {
            ...
            try {
                const blob = new Blob(
                    [JSON.stringify({ sessionId: sid || '', visitorId: vid })],
                    { type: 'application/json' }
                );
                navigator.sendBeacon(`${API_BASE}/api/reception-demo/end`, blob);
            } catch (_e) {
    ...
    const displayName = useMemo(() => {
        if (!receptionLead) return 'Demo visitor';
        const n = String(receptionLead.fullName || '')
            .trim() ||
            [receptionLead.firstName, receptionLead.lastName]
                .map((s) => (s || '').trim())
                .filter(Boolean)
                .join(' ')
                .trim();
        return n || 'Demo visitor';
    }, [receptionLead]);
    const displayPosition = 'Evaalo reception';

    const loadingCandidate = false;
    ...
    } = useLiveKitToken(visitorId, sessionId);
    const { 
        agentState, 
        customState, 
        sendState, 
    ...
    const startInterview = async (e) => {
        ...
        const leadPayload = receptionLeadRef.current;
        if (
            !leadPayload ||
            !String(leadPayload.firstName || '').trim() ||
            !String(leadPayload.lastName || '').trim() ||
            !String(leadPayload.email || '').trim()
        ) {
            alert('Please complete your details to start the demo.');
            return;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(leadPayload.email).trim())) {
        ...
        try {
                ...
                if (!settings.echoCancellation || settings.echoCancellation === false) {
                    ...
                    try {
                        ...
                        if (newSettings.echoCancellation) {
                            ...
                            alert('❌ تحذير حرج: إلغاء الصدى غير مفعّل ولا يمكن تفعيله.\n\n💡 الحل الوحيد: استخدم سماعات الرأس (headphones) لمنع الصدى تماماً.\n\n⚠️ بدون سماعات الرأس، سيكون هناك صدى قوي.');
                        }
                    } catch (constraintError) {
                        console.error('❌ CRITICAL: Failed to apply echo cancellation constraints:', constraintError);
                        alert('❌ تحذير حرج: فشل تفعيل إلغاء الصدى.\n\n💡 الحل الوحيد: استخدم سماعات الرأس (headphones) لمنع الصدى تماماً.');
                    }
                } else {
            ...
            const startResponse = await fetch(`${API_BASE}/api/reception-demo/start`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    visitorId,
                    firstName: String(leadPayload.firstName).trim(),
                    lastName: String(leadPayload.lastName).trim(),
                    email: String(leadPayload.email).trim(),
            ...
            if (abortStartIfStale('after /api/reception-demo/start')) {
                return;
            }
            ...
            if (startData.livekit && startData.livekit.roomName && startData.livekit.token && startData.livekit.url) {
                try {
                    ...
                    alert('❌ LiveKit connection failed. Please check console for details. Interview cannot proceed.');
                ...
                console.error('❌ LiveKit is REQUIRED. Interview cannot proceed without LiveKit.');
                alert('❌ LiveKit not available. Please check backend configuration. Interview cannot proceed.');
            ...
            if (error.message?.includes('getUserMedia')) {
                ...
                errorMessage += 'Check your internet connection and make sure the backend is running.';
            } else if (error.message?.includes('404')) {
                errorMessage += 'Demo session could not be started. Try again.';
            } else {
            ...
            setPreparationTime(PREPARATION_COUNTDOWN_SECONDS > 0 ? PREPARATION_COUNTDOWN_SECONDS : 0);
            setAvatarVideoSurfaceReady(false);
            setReceptionLead(null);
            receptionLeadRef.current = null;
        } finally {
            isStartingRef.current = false;
        }
    };

    const handleReceptionLeadStart = async (e) => {
        if (e) {
            e.preventDefault();
            e.stopPropagation();
        }
        setLeadFormError('');
        const fullName = leadDraft.fullName.trim();
        const email = leadDraft.email.trim();
        const company = leadDraft.company.trim();
        if (!fullName) {
            setLeadFormError(t('receptionDemoLead_errorFullName'));
            return;
        }
        if (!email) {
            setLeadFormError(t('receptionDemoLead_errorEmail'));
            return;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            setLeadFormError(t('receptionDemoLead_errorEmailInvalid'));
            return;
        }
        const { firstName, lastName } = splitFullNameForLead(fullName);
        const lead = { fullName, firstName, lastName, email, company };
        receptionLeadRef.current = lead;
        setReceptionLead(lead);
    ...
    const endInterview = async (options = {}) => {
        ...
        try {
            ...
            // إرسال طلب إنهاء الديمو إلى Backend — visitorId احتياطي إذا فقد sessionId
            if (sessionId || visitorIdRef.current) {
                try {
                    const response = await fetch(`${API_BASE}/api/reception-demo/end`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                ...
                console.warn('⚠️ No sessionId or visitorId to end reception demo');
            }
    ...
    if (!isInterviewActive && receptionLead === null) {
        return (
            <main className="reception-page" dir={isRtl ? 'rtl' : 'ltr'}>
                ...
                <div
                    className="reception-page__card reception-demo-lead-modal"
                    role="dialog"
                    aria-labelledby="reception-demo-lead-title"
                >
                    <div className="reception-page__card-header">
                        ...
                        <h1 id="reception-demo-lead-title" className="reception-page__title">
                            {t('receptionDemoLead_title')}
                        </h1>
                    ...
                    <div className="reception-page__card-body">
                        ...
                        >
                            <p className="reception-demo-lead-form__lead">{t('receptionDemoLead_enterDetails')}</p>
                            {leadFormError ? (
                                <p className="reception-page__error" role="alert">
                                    {leadFormError}
                                </p>
                            ) : null}
                            <form onSubmit={handleReceptionLeadStart}>
                                <div
                                    className="reception-demo-lead-form-grid"
                                    style={{
                                        width: '100%',
                                        margin: '0 auto',
                                        display: 'grid',
                                ...
                                >
                                    {[
                                        {
                                            k: 'fullName',
                                            labelKey: 'receptionDemoLead_fullName',
                                            type: 'text',
                                            auto: 'name',
                                            gridColumn: 'span 2',
                                        },
                                        {
                                            k: 'email',
                                            labelKey: 'receptionDemoLead_email',
                                            type: 'email',
                                            auto: 'email',
                                            gridColumn: 'span 2',
                                        },
                                        {
                                            k: 'company',
                                            labelKey: 'receptionDemoLead_company',
                                            type: 'text',
                                            auto: 'organization',
                                            gridColumn: 'span 2',
                                        },
                                    ].map(({ k, labelKey, type, auto, gridColumn }) => (
                                        <label
                                            key={k}
                                            style={{
                                                display: 'block',
                                        ...
                                        >
                                            ...
                                            <input
                                                ...
                                                value={leadDraft[k]}
                                                onChange={(ev) =>
                                                    setLeadDraft((d) => ({ ...d, [k]: ev.target.value }))
                                                }
                                                placeholder={k === 'company' ? t('receptionDemoLead_optional') : ''}
                                                className="reception-demo-lead-input"
                                            />
                                        </label>
                                    ))}
                                </div>
                                <div className="reception-page__actions">
                                    <button
                                        type="submit"
                                        className="reception-page__btn reception-page__btn--primary reception-page__btn--idle-glow"
                                    >
                                        {t('reception_start')}
                                    </button>
                                </div>
    ...
    return (
        <div style={{
            ...
            {/* Background Effects — نفس أسلوب توهج AvatarShowcase */}
            <div style={{
                position: 'absolute',
                top: 0,
                left: 0,
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <h1
                        ...
                        fontWeight: 700,
                            margin: 0,
                            background: 'linear-gradient(135deg, #38bdf8 0%, #c084fc 55%, #a78bfa 100%)',
                ...
                {countdownActive && (
                    <div style={{
                        ...
                        <div style={{
                                fontSize: '120px',
                            fontWeight: 700,
                            background: 'linear-gradient(135deg, #38bdf8 0%, #22d3ee 40%, #c084fc 100%)',
                            WebkitBackgroundClip: 'text',
                            WebkitTextFillColor: 'transparent',
                            backgroundClip: 'text',
                                marginBottom: '24px',
                        ...
                        <div
                            ...
                            style={{
                                ...
                                borderTopColor: 'rgba(192, 132, 252, 0.92)',
...
export default ReceptionDemoCall;
