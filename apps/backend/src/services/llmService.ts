...
export interface SelectedQuestion {
    ...
    topic?: string;
    /**
     * مفتاح موضوع المرحلة الثانية المطروح — يُسجَّل في الحالة فلا يُعاد.
     * منفصل عن `topic` لأنّ ذاك يختار وضع بناء الموجّه، وهذا محاسبةٌ فقط. الخلط
     * بين الغرضين هو ما جعل نصّ السؤال الإلزامي يُرمى (انظر فرع وضع الموضوع).
     */
    topicKey?: string;
    /**
...
function sanitizeVoiceReply(text: string, ack: number | LLMContext = 0): string {
    ...
    // والمذكّرة كذلك: «تعاملت ويه» ← «وياه». غطّيتُ المؤنّثة وحدها أولاً، فظهرت
    // المذكّرة في جلسة d9eb5536 («وكيف تعاملت ويه؟»). لا تُطابق «وية» — تلك
    // حرف الجرّ المجرّد وهو صحيح، والفرق بينهما التاء المربوطة.
    s = s.replace(/(?<!\p{L})ويه(?!\p{L})/gu, 'وياه');
