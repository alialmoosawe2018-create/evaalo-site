...
export interface CompareReportPdfMeta {
    ...
    generatedAt?: Date;
    /**
     * candidateId → data URI. Chromium renders this HTML with no session, so a
     * `/uploads/...` src would come back empty — the bytes must be inline.
     * See services/compareReportPhotos.ts.
     */
...
/**
 * Two letters from the name, for candidates with no photo on file. Better than an
 * empty circle, and it keeps every card the same shape so the column of avatars
 * stays aligned down the page.
 */
...
function avatarHtml(item: RankingItem, name: string, photos?: Record<string, string>): string {
    const id = typeof item.candidateId === 'string' ? item.candidateId.trim() : '';
    const src = id && photos ? photos[id] : undefined;
    if (src) {
        // The data URI is generated by us, never by the candidate, so it is not
        // escaped as text — but the alt attribute is candidate-controlled.
        return `<img class="avatar" src="${src}" alt="${esc(name)}" />`;
    }
    return `<span class="avatar avatar--initials">${esc(initialsOf(name))}</span>`;
}
...
export function buildCompareReportHtml(
    ...
    const cards = ranking.map((item, i) => candidateCard(item, i, meta.photos)).join('\n');
...
<style>
  * { box-sizing: border-box; }
  ...
  .avatar {
    ...
    border: 2px solid #ffffff; box-shadow: 0 0 0 1px #cbd5e1; background: #e2e8f0;
  }
  .avatar--initials {
    display: inline-flex; align-items: center; justify-content: center;
    font-weight: 800; font-size: 13px; color: #475569; letter-spacing: .5px;
  }
