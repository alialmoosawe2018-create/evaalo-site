...
/**
 * المحاور الإضافية — سببها أنّ الجلسة الإنجليزية تعيش أطول من بنكها.
 *
 * الجلسة الإنجليزية لا مرحلة ثالثة لها (`interviewController`: اختبار إنجليزية
 * داخل مقابلة إنجليزية بالكامل بلا معنى)، فالمرحلة الثانية تبقى مفتوحة حتى سقف
 * الوقت. والمحاور الستّة تنفد قبل ذلك بكثير، فيدخل احتياطُ `pickPhase2Topic`
 * ويعيد البنك بالترتيب نفسه — وهذا ما سُمع في الجلستين `6cfb7d62` و`2718fd5f`
 * (٢٠٢٦-٠٩-٢٣): الدورات والدراسة والشركة واللغات مرّتين، والعمل الجماعي ثلاثاً.
 *
 * ⚠️ **ولا يكفي توسيع البنك وحده، مهما وُسّع.** طول المقابلة محكوم بمؤقّت اثنتي
 * عشرة دقيقة لا بعدد أدوار، والمرحلة الثانية قد تُفتح عند الدور الثامن لا الثاني
 * عشر (`leavePhase1Early` حين تُغطّى محاور المرحلة الأولى السبعة). مقيس بإعادة
 * تشغيل التدفّق الحقيقي: ٢٦ دوراً ⇒ ١٨ دور محاور مقابل ١٥ مفتاحاً صالحاً ⇒ ثلاث
 * إعادات؛ و٣٤ دوراً ⇒ عشر. فالبنك يؤخّر النفاد، ومَن يعالجه هو مسار **التعميق**
 * في `selectNextQuestion` (سؤال أعمق عمّا قاله المرشّح، بلا نصٍّ يتكرّر).
 *
 * ⚠️ **وكلّ محور هنا فُحص نصّاً بنصّ ضدّ الخمسين سؤالاً في `POOL_QUESTIONS`.**
 * المحاولة الأولى ضمّت `mistake` و`feedback` و`conflict` و`pressure` و`motivation`
 * — وكلّها مطروحة أصلاً في بنوك المرحلة الأولى (p7 «مرّة غلطت بشغلك»، p7 «آخر
 * ملاحظة وصلتك»، p3 «شلون تتعامل وية الخلاف»، p3 «ضغط الوقت»، p1 «شنو حمسك
 * تقدّم؟»). أي أنّ «علاج التكرار» كان سيعيد إنتاجه بصيغة أخرى. فحُذفت الخمسة،
 * وما بقي محاور لا يلمسها أيّ بنك: الحجم، وأوّل الأسابيع، والغموض، وتدريب
 * الآخرين، وما يحتاجه من صاحب العمل.
 *
 * ولماذا سلوكية؟ لأنّ المرحلة الثانية تقيس الشخصية والسلوك (لا الكفاءات — تلك
 * للمرحلة الثالثة بالفيديو)، وأبعاد المقيّم السبعة تُقاس من هذا النوع بالضبط.
 */
export const PHASE2_TOPIC_KEYS_EXTRA = [
    'achievement',
    'scale',
    'onboarding',
    'ambiguity',
...
/**
 * محاور المرحلة الثانية — بنكٌ واحد للجميع، الستّة الأساسية أوّلاً ثمّ العشرة.
 *
 * ⚠️ لا يتفرّع على اللغة، وهذا **تصحيحٌ لمحاولةٍ سابقة تفرّعت**. كان البنك
 * الممتدّ للإنجليزية وحدها، وللمرشّح أن يحوّل الجلسة إلى العربية في منتصفها
 * (`isWantsArabicSwitch`) — فينكمش البنك من ستّة عشر إلى ستّة، وتكون الستّة كلّها
 * مطروحة، فيسقط فوراً في احتياط الدوران: تكرارٌ من أوّل دور بعد التحويل.
 *
 * وتوحيدُه آمنٌ على العربية: مرحلتها الثانية أربعة أدوار أو خمسة، والاختيار
 * `unasked[0]` يبدأ من الأساسية دائماً، فلا تبلغ الإضافات أصلاً.
 */
export function getPhase2TopicKeys(): readonly string[] {
  return [...PHASE2_TOPIC_KEYS, ...PHASE2_TOPIC_KEYS_EXTRA];
}
