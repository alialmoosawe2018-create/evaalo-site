...
/**
 * ⚠️ الصيغ المؤنّثة ليست زينة هنا — صوت الوكيل أنثويّ، فالمرشّحون يخاطبونه بالمؤنّث.
 *
 * من الجلسة `d05890bc`: طلب علي رياض التوضيح مرّتين ولم يُفهَم مرّتين، ثمّ قال
 * «سؤالك… خلي نقول سلس وبسيط» واستسلم. السبب أنّ القائمة كانت مذكّرة:
 *   «شنو تقصد» تُطابق، و«تقصدين؟» مجرّدةً لا تُطابق (لا «شنو» قبلها).
 *   «عيد السؤال» تُطابق، و«اعيدي السؤال» لا (الياء تكسر التسلسل).
 *   «مثال على شنو؟» لم تكن موجودة إطلاقاً.
 * نفس عائلة `\b` الميّتة بجوار العربية: قاعدةٌ تبدو عاملة وهي عمياء عن نصف الناس.
 *
 * والحدود `(?<!\p{L})…(?!\p{L})` لا `\b` — راجع [[arabic-word-boundary]] في
 * `arabic-word-boundary-audit.ts`: `\b` بجوار العربية شرطٌ ميّت لا يتحقّق.
 */
const CLARIFICATION_PATTERNS: RegExp[] = [
  // القائمة الأصلية (مذكّرة أو محايدة)
  /(ما\s+فهمت|مو\s+واضح(?:لي)?|شنو\s+تقصد|شنو\s+المقصود|وضح(?:لي|\s+لي)?|ممكن\s+توضح(?:لي)?|اشرح(?:لي)?|ممكن\s+تشرح|عيد\s+السؤال|ممكن\s+تعيد(?:\s+السؤال)?|clarify|explain|what do you mean|can you explain|repeat)/i,
  // «تقصدين؟» / «تقصدون» / «قصدك» — مجرّدةً بلا «شنو»
  /(?<!\p{L})(?:تقصدين|تقصدون|تقصد|قصدك|قصدج)(?!\p{L})/u,
  // «اعيدي السؤال» — الأمر بصيغة المؤنّث
  /(?<!\p{L})(?:ا|أ)?عيدي(?!\p{L})/u,
  // «مثال على شنو؟» / «تقصدين على شنو» — استيضاح موضوع الطلب لا صياغته
  /(?<!\p{L})على\s+شنو(?!\p{L})/u,
  /(?<!\p{L})شنو\s+المطلوب(?!\p{L})/u,
  // «مو فاهم/فاهمة» / «ما افتهمت»
  /(?<!\p{L})(?:مو|ما)\s*(?:فاهم|فاهمة|افتهمت|فهمتك)(?!\p{L})/u,
];
  ...
  if (phase === 1) {
    if (mandatoryQuestionDue) {
      ...
      return {
        ...
        pool: 0, // mandatory
        // السؤال الإلزامي يُطرح على كلّ مرشّح ليقف الجميع على أرض واحدة، فنصُّه
        // ليس اقتراحاً. بدون هذا العلَم كان `topic` أدناه يفوز على `text` في
