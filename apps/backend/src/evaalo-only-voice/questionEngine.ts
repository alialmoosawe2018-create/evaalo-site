...
export function isAskingAgentIdentity(transcript: string): boolean {
  ...
  const ar =
    /(من|مين|منو|شلون|شو)\s+(أنت|انته|انتا|انتم|أنتم|انت|انته|أنتن)\b/i.test(t) ||
    /^أنت\s+مين\b|^(انت|انته|انته)\s+مين\b|^(مين|شنو) أ(ن|)ت(م|ن|)\b/i.test(t) ||
    /(ممكن|اريد|أريد|بدي|بعد|خلينا)\s+تعرف(ني|ينا|وني|يني|ينا)/i.test(t) ||
    /(عرف(ني|ينا|وني|يني|نا|يني|ينا)|ممكن (ت|)عرف(ني|ينا|وني|يني|ينا)|(ت|)خبر(ني|ينا|يني|ينا))\s+عن\s+نفس(ك|چك|چي|ي)\b/i.test(
      t
    ) ||
    /(ت|)(ح|)چي\s+ل(ي|نا)\s+عن\s+نفس(ك|چك|ك)\b|تكلم(يني|ينا|ي)?\s+عن\s+نفس(ك|چك|ك)\b/i.test(
      t
    ) ||
    /(شنو|شو)\s+طبيعت(ك|چك|ك|ج)\b|وين (أنت|انته) من\??/i.test(t);
...
export function classifyInterviewPolicyIntent(transcript: string): InterviewPolicyIntent | null {
  ...
  const opinAr =
    /(رأيك|رأیك|رأی)\s+ب(يا|ي|يَه|يَه|يه|نفس(ي|ي|يَه|ك)|مستواي|أداءي|مستوى|أدائي|شلون(ي|ي|چي)|وين(ي|ي|چي)|شلون(ني|ی)|شلون(چي|شي))\b/i.test(
      t
    ) || /(شنو|شو)\s+رأي(ك|چك)\s*ب(يا|ي|يه|نفس(ي|ي|ك|چي))\b/i.test(t);
  const opinEn =
  ...
  const evalAr =
    /(قيم(يني|ينا|وني|يني|ينا|ني)|تقيم(يني|يني|ني)|تقي(يد|يم)(ني|ي|ك)|تقي(يد|يم)\s*اجاب(ات|ي|يَه)|شلون\s*(تقيم|تقي(يد|يم))(ني|يني|ينا)|شنو\s*(تقيم(ي|ي|ك)|تقي(يد|يم)(ي|ك)))\b/i.test(
      t
    ) ||
    /(تقيم(ي|ي|ك)|تقي(يد|يم)(ي|ك)|نقاط(ي|ي|ك)|درج(ات|ي)ي|شون\s*كنت\s*ب(المعايير|تقيي(م|م)))\b/i.test(
      t
    );
  const evalEn =
  ...
  const resAr =
    /(ال)?نتيج(ة|ه|ۀ|ه)\b|ناتج(؟|ي|ٱ)?\b|متى (تطلع|تنزل|نظهر|ينعلن|يعلن|يوصل) (ال)?نتيج|شصار(ت)? (ال)?نتيج|وين (صارت|وصلت) (ال)?نتيج|انقبل|قبول|راح انقبل|راح (أ|ا)نقبل|تعرف(ني|يني) (بشكل|ناجح|منقبل|مرفوض|مقبول)/i.test(
      t
