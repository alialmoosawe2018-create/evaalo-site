...
import { getLLMResponse, getTimeEndedClosingMessage, getTimeEndedApologyMessage, getInitialGreetingMessage, type InterviewPhase } from "../services/llmService.js";
...
import { buildConversationMp3, type RecordingSegment } from "../services/voiceRecordingService.js";
import { uploadBuffer, isR2Configured } from "../services/r2Service.js";
...
/**
 * دمج مقاطع المحادثة إلى MP3 ورفعها إلى R2 ثم حفظ الرابط على المرشح.
 * تعمل بعد إغلاق الجلسة (لا تحجب الاتصال). الأخطاء تُسجَّل فقط دون إيقاف النظام.
 */
async function finalizeRecording(
  ...
  try {
    ...
    // مفتاح منظّم حسب المؤسسة لمنع التصادم وتسهيل الحذف/سياسات الاحتفاظ.
...
export function handleVoiceWsConnection(ws: WebSocket, req: IncomingMessage) {
  ...
  // المسار العام (رابط مشارَك): mode=public => حقن معايير الوظيفة في الايجنت + إرسالها مع الترانسكريبت.
  ...
  // معايير الوظيفة + الإعلان للحملة العامة؛ تُحمَّل عند بدء الجلسة وتُستخدم في البرومت وعند إرسال n8n.
  ...
  const loadPublicCampaignContext = async (): Promise<PublicCampaignContext> => {
    try {
      ...
      return {
        jobCriteria: (camp as any).criteria || undefined,
  ...
  console.log(`[SESSION START] ${sessionId.substring(0, 8)}... candidateId: ${candidateId || "none"} language: ${language || "auto"}${sessionMode === "public" ? " mode: public" : ""}`);

  // تحميل معايير الحملة للمقابلة العامة — Promise يُعاد انتظاره عند الإغلاق لضمان إرفاقها مع الترانسكريبت في n8n.
  const publicCampaignContextPromise =
    sessionMode === "public"
      ? loadPublicCampaignContext().then((ctx) => {
          applyPublicCampaignContext(ctx);
          console.log(
            `[PUBLIC] ${sessionId.substring(0, 8)}... loaded campaign ${ctx.resolvedCampaignId || "none"} criteriaKeys=${jobCriteria ? Object.keys(jobCriteria).length : 0} jobAd=${jobAdvertisement ? "yes" : "no"}`
          );
          return ctx;
        })
      : undefined;
  ...
  // يُفعَّل فقط عند ضبط إعدادات R2 ووجود candidateId صالح، ولا يؤثر على بقية المنطق.
  ...
  const recordChunk = (speaker: "user" | "agent", format: "pcm" | "mp3", chunk: Buffer) => {
    ...
    if (recordingBytes + chunk.length > recordingMaxBytes) return; // حدّ أمان لمنع تضخّم الذاكرة
  ...
  /** انتظار توقف المستخدم عن الكلام قبل إرسال للـ LLM (قابل للضبط: VOICE_USER_STOPPED_MS) */
  ...
  const runPipeline = async (transcript: string) => {
    ...
    if (timeEndedSent) {
      ...
      try {
        ...
        const sendChunk = (c: Buffer) => { recordChunk("agent", "mp3", c); send(ws, { type: "audio_chunk", chunkBase64: c.toString("base64"), format: "mp3" }); };
    ...
    try {
      ...
      if (selectedQuestion?.isFixed) {
        llmReply =
          ...
          (candidateLastLang === 'ar' ? 'ممكن تحچيلي أكثر عن هالموضوع؟' : 'Could you tell me more about that?');
      ...
      // لغة TTS من رد الإيجنت فقط: عربي → صوت عربي، إنجليزي → صوت إنجليزي (ثابت)
      const ttsLanguage = /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF]/.test(llmReply.trim()) ? ('ar' as const) : ('en' as const);

      // تحويل character alignment إلى كلمات مع أوقات البداية
      ...
      if (ws.readyState !== ws.OPEN) return;
      // المقابلة انتهت: الإيجنت يغلق الاتصال من جهته بعد سماع الرسالة الختامية كاملة
      if (selectedQuestion?.isInterviewEnd === true) {
      ...
      case "start_listening": {
        ...
        (async () => {
          try {
            ...
            const greetingMsg = await getInitialGreetingMessage({
  ...
  ws.on("close", () => {
    ...
    speechBuffers.delete(sessionId);
    const history = conversationHistory.get(sessionId);
    const historyCopy = history && history.length > 0 ? [...history] : null;
    const interviewState = getInterviewState(sessionId);
    ...
    // إنهاء التسجيل: دمج المقاطع → MP3 → رفع R2 → تحديث المرشح (غير حاجب لإغلاق الاتصال).
    if (recordingEnabled && recordingSegments.length > 0) {
      const segments = recordingSegments.splice(0);
      void finalizeRecording(sessionId, candidateId, segments);
    }
