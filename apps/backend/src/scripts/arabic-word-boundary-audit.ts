...
// الأفعال التي كانت خارج المطابقة، فبقيت مذكّرة أمام مرشّحة بينما بقيّة المقابلة
// تخاطبها بالمؤنّث — التنافر داخل المقابلة الواحدة أوضح للأذن من خطأٍ مطّرد.
// النصوص أدناه هي الاحتياطيّات الحقيقية كما وردت في جلسة 6afff73c.
check(
    'أنثى: «تفضل تشتغل» → مؤنّث (نصّ محور العمل الجماعي)',
    /تفضلين/.test(applyIraqiGenderPhrasing('شلون تفضل تشتغل لوحدك او مع فريق؟', 'female')),
    true,
);
check(
    'أنثى: «تستخدمها» → «تستخدمينها»',
    /تستخدمينها/.test(applyIraqiGenderPhrasing('شنو البرامج اللي تستخدمها بلعمل؟', 'female')),
    true,
);
check(
    'أنثى: «تتعامل» → «تتعاملين»',
    /تتعاملين/.test(applyIraqiGenderPhrasing('شلون تتعامل وية ضغط العمل بشغلك؟', 'female')),
    true,
);
check(
    'ذكر: «تفضل تشتغل» تبقى كما هي',
    applyIraqiGenderPhrasing('شلون تفضل تشتغل لوحدك او مع فريق؟', 'male').includes('تفضل تشتغل'),
    true,
);
// الغائب المؤنّث ليس مخاطَباً: «الشركة تتعامل» يجب ألّا تصير «تتعاملين». هذا هو
// سبب حصر القائمة بأفعال بعينها بدل إضافة «ين» لكلّ ما يبدأ بتاء.
check(
    'الغائب المؤنّث لا يُحوَّل',
    /تتعاملين/.test(applyIraqiGenderPhrasing('الشركة تتعامل مع الزبائن', 'male')),
    false,
);
